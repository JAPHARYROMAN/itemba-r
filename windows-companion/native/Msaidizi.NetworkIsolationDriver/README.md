...
The Egress Supervisor cannot and must not open this device. Its service SID is deliberately absent from the device DACL and per-request token allowlist; this v3 driver does not implement the egress supervisor's `MSAIDIZI-EGRESS-DRIVER-HEALTH-V1` protocol. Do not widen the DACL or repurpose this device to satisfy an egress posture probe.
