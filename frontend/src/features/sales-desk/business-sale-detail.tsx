...
import { useWorkspaceRouter as useRouter } from '@/components/workspace/workspace-navigation';
import {
  Btn,
  Card,
  EmptyState,
...
export function BusinessSaleDetail({ saleId }: { saleId: string }) {
        ...
        actions={
          <div className="flex flex-wrap gap-2">
            ...
            {order.customer?.id && (
              <Btn
                variant="secondary"
                onClick={() => router.push(`/sales-desk/customers/${order.customer.id}`)}
              >
                View Customer
              </Btn>
            )}
            ...
            {order.status === 'DRAFT' && hasPermission('sales.create') && (
              <Btn
                variant="secondary"
                onClick={() => router.push(`/sales-desk/sales?editId=${order.id}`)}
              >
                Edit Draft
              </Btn>
            )}
