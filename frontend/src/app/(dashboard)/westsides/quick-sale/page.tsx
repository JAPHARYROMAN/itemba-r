...
interface InventoryLocation { id: string; name: string; locationCode: string; branchId?: string | null }
...
interface Customer { id: string; name: string; customerCode?: string | null }
...
function fmt(n: number | string | undefined | null): string {
  return new Intl.NumberFormat('en-TZ', { minimumFractionDigits: 2, maximumFractionDigits: 2 }).format(Number(n ?? 0));
}
...
export default function QuickSalePage() {
  ...
  useEffect(() => {
    if (!hydrated) return;
    if (!settings.companyId || !settings.branchId || !settings.inventoryLocationId || !settings.cashAccountId) {
      setSettingsOpen(true);
    }
  }, [hydrated, settings.companyId, settings.branchId, settings.inventoryLocationId, settings.cashAccountId]);
  ...
  useEffect(() => {
    fetch('/api/backend/companies?limit=100').then(r => r.json())
      ...
      .catch(() => setCompanies([]));
    fetch('/api/backend/units?limit=200').then(r => r.json())
      .then(j => setUnits(Array.isArray(j.data?.data) ? j.data.data : Array.isArray(j.data) ? j.data : []))
      .catch(() => setUnits([]));
  }, []);
  ...
  useEffect(() => {
    if (!settings.companyId) {
      setBranches([]); setDivisions([]); setLocations([]); setCashAccounts([]); setCustomers([]);
      return;
    }
    const cid = settings.companyId;
    fetch(`/api/backend/branches?companyId=${cid}&limit=200`).then(r => r.json())
      .then(j => setBranches(Array.isArray(j.data?.data) ? j.data.data : Array.isArray(j.data) ? j.data : []))
      .catch(() => setBranches([]));
    fetch(`/api/backend/divisions?companyId=${cid}&limit=200`).then(r => r.json())
      .then(j => setDivisions(Array.isArray(j.data?.data) ? j.data.data : Array.isArray(j.data) ? j.data : []))
      .catch(() => setDivisions([]));
    fetch(`/api/backend/inventory-locations?companyId=${cid}&limit=200`).then(r => r.json())
      .then(j => setLocations(Array.isArray(j.data?.data) ? j.data.data : Array.isArray(j.data) ? j.data : []))
      .catch(() => setLocations([]));
    fetch(`/api/backend/cash-accounts?companyId=${cid}&limit=200`).then(r => r.json())
      .then(j => setCashAccounts(Array.isArray(j.data?.data) ? j.data.data : Array.isArray(j.data) ? j.data : []))
      .catch(() => setCashAccounts([]));
    fetch(`/api/backend/customers?companyId=${cid}&limit=500`).then(r => r.json())
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      try {
        ...
        const list: Product[] = Array.isArray(j.data?.data) ? j.data.data : Array.isArray(j.data) ? j.data : [];
  ...
  const totals = useMemo(() => {
    const subtotal = cart.reduce((s, l) => s + (Number(l.qty) || 0) * (Number(l.unitPrice) || 0), 0);
  ...
  const settingsReady = !!(settings.companyId && settings.branchId && settings.inventoryLocationId && settings.cashAccountId);
  ...
  return (
    <div className="p-6 quick-sale-page">
      ...
      <Card className="p-3 mb-4 flex flex-wrap items-center gap-3 text-sm">
        ...
        <Btn variant="secondary" size="xs" onClick={() => setSettingsOpen(true)}>Settings</Btn>
      </Card>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4">
        ...
        <div className="lg:col-span-3 space-y-4">
          <Card className="p-4">
            <label className="block text-xs uppercase tracking-wide text-slate-500 mb-1">Add product (name, SKU, or scan barcode)</label>
            <div className="relative">
              ...
              {showResults && productResults.length > 0 && (
                <ul className="absolute z-10 left-0 right-0 mt-1 max-h-72 overflow-y-auto rounded-lg border bg-white shadow-lg" style={{ borderColor: 'var(--aurora-border)' }}>
              ...
              {productSearching && <div className="absolute right-3 top-3 text-xs text-slate-400">…</div>}
            </div>
            <p className="text-[11px] text-slate-500 mt-1.5">Press <kbd className="px-1 bg-slate-100 rounded">Enter</kbd> to add the first match, <kbd className="px-1 bg-slate-100 rounded">Esc</kbd> to dismiss.</p>
          </Card>
            ...
            <table className="w-full text-sm">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr style={{ color: 'var(--aurora-text-muted)' }}>
                  <th className="px-3 py-2 text-left text-xs font-semibold uppercase tracking-wide">Item</th>
              ...
              <tbody>
                {cart.length === 0 && (
                  <tr><td colSpan={5} className="text-center py-8 text-sm text-slate-400 italic">No items yet — search above to add</td></tr>
                )}
                {cart.map((l, i) => (
                  <tr key={i} className="border-b border-slate-50">
                    ...
                    <td className="px-3 py-2 text-right">
                      <div className="inline-flex items-center gap-1">
                        <button onClick={() => updateLine(i, { qty: Math.max(0.001, l.qty - 1) })}
                          className="w-7 h-7 rounded border text-slate-600 hover:bg-slate-100" style={{ borderColor: 'var(--aurora-border)' }}>−</button>
                        <input type="number" value={l.qty} step="any" min="0.001"
                        ...
                        <button onClick={() => updateLine(i, { qty: l.qty + 1 })}
                          className="w-7 h-7 rounded border text-slate-600 hover:bg-slate-100 ml-1" style={{ borderColor: 'var(--aurora-border)' }}>+</button>
                      </div>
                    </td>
                    <td className="px-3 py-2 text-right">
                      <input type="number" value={l.unitPrice} step="any" min="0"
                        onChange={(e) => updateLine(i, { unitPrice: Number(e.target.value) || 0 })}
                        className="w-24 text-right border rounded px-2 py-1 tabular-nums"
                        style={{ borderColor: 'var(--aurora-border)' }} />
                    </td>
                    <td className="px-3 py-2 text-right tabular-nums font-semibold">{fmt(l.qty * l.unitPrice)}</td>
                    <td className="px-3 py-2 text-right">
                      <button onClick={() => removeLine(i)} className="text-slate-400 hover:text-red-600 text-lg leading-none">×</button>
                    </td>
                  </tr>
                ))}
              </tbody>
          ...
          <Card className="p-4 space-y-3">
            <div>
              ...
              <FormSelect
                ...
                placeholder="Walk-in"
                options={customers.map(c => ({ value: c.id, label: c.customerCode ? `${c.name} — ${c.customerCode}` : c.name }))}
              />
            </div>
            {!customerId && (
              <FormInput label="Walk-in name (optional)" value={walkInName} onChange={(e) => setWalkInName(e.target.value)} placeholder="—" />
            )}
            {settings.paymentMethod !== 'CASH' && (
              <FormInput label="Payment reference" value={paymentReference} onChange={(e) => setPaymentReference(e.target.value)} placeholder="M-Pesa code, slip #" />
            )}
          </Card>
      ...
      <Modal
        ...
        footer={<Btn variant="primary" onClick={() => setSettingsOpen(false)} disabled={!settingsReady}>{settingsReady ? 'Done' : 'Fill all fields'}</Btn>}
      >
        <div className="space-y-3">
          <FormSelect
            ...
            onChange={(e) => setSettings(s => ({ ...s, companyId: e.target.value, branchId: '', divisionId: '', inventoryLocationId: '', cashAccountId: '' }))}
          ...
          <FormSelect
            ...
            options={branches.map(b => ({ value: b.id, label: `${b.code ? b.code + ' — ' : ''}${b.name}` }))}
          ...
          <FormSelect
            ...
            options={[
              ...
              ...divisions.map(d => ({ value: d.id, label: `${d.code ? d.code + ' — ' : ''}${d.name}` })),
            ]}
          ...
          <FormSelect
            ...
            options={locations.map(l => ({ value: l.id, label: `${l.locationCode ? l.locationCode + ' — ' : ''}${l.name}` }))}
          ...
          />
          <p className="text-xs text-slate-500">These settings are saved on this device. Change them anytime.</p>
        </div>
      </Modal>
      ...
      {confirmed && (
        <Modal
          ...
          footer={
            <>
              <Btn variant="secondary" onClick={startNew}>New Sale</Btn>
      ...
      <style jsx global>{`
        @media print {
          ...
          .quick-sale-receipt, .quick-sale-receipt * { visibility: visible !important; }
          .quick-sale-receipt {
...
function Row({ label, value, highlight, muted }: { label: string; value: string; highlight?: boolean; muted?: boolean }) {
  return (
    <div className={`flex items-center justify-between ${highlight ? 'text-lg font-bold' : muted ? 'text-slate-500' : ''}`}>
      <span>{label}</span>
...
function SettingChip({ label, value }: { label: string; value?: string }) {
  return (
    <div className="text-xs">
      ...
      <span className={`font-medium ${value ? '' : 'text-amber-600 italic'}`}>{value ?? '— set me —'}</span>
    </div>
  );
}
  ...
  return (
    <div className="quick-sale-receipt text-sm">
      <div className="text-center mb-3">
        ...
        <div className="text-xs text-slate-500">{new Date(order.orderDate).toLocaleString('en-GB')}</div>
      </div>
      <div className="border-t border-b border-slate-300 py-2 my-2 text-xs">
        <div className="flex justify-between"><span>Sale #</span><span className="font-mono">{order.salesOrderNumber}</span></div>
        <div className="flex justify-between"><span>Customer</span><span>{order.customer?.name ?? order.customerName ?? 'Walk-in'}</span></div>
        {cashier && <div className="flex justify-between"><span>Cashier</span><span>{cashier}</span></div>}
      </div>
        ...
        <tbody>
          {order.lines.map((l, i) => (
            <tr key={i}>
              ...
              <td className="text-right py-0.5">{Number(l.quantity)} {l.unit?.symbol ?? ''}</td>
      ...
      <div className="border-t border-slate-300 pt-2 mt-2 space-y-1 text-xs">
        <div className="flex justify-between"><span>Subtotal</span><span className="tabular-nums">TZS {fmt(order.subtotal)}</span></div>
        {Number(order.taxAmount) > 0 && <div className="flex justify-between"><span>Tax</span><span className="tabular-nums">TZS {fmt(order.taxAmount)}</span></div>}
        {Number(order.discountAmount) > 0 && <div className="flex justify-between"><span>Discount</span><span className="tabular-nums">−TZS {fmt(order.discountAmount)}</span></div>}
        <div className="flex justify-between text-base font-bold pt-1"><span>TOTAL</span><span className="tabular-nums">TZS {fmt(order.totalAmount)}</span></div>
      </div>
      <div className="text-center text-xs mt-4 text-slate-500">
        Paid by {order.paymentMethod}{order.paymentReference ? ` (${order.paymentReference})` : ''} — {order.cashAccount?.accountName ?? ''}
      </div>
