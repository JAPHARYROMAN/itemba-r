...
export default function SecurityDashboardPage() {
  ...
  useEffect(() => {
    backendGet<any>('security/dashboard')
      .then((d) => {
        const totalEvents = Array.isArray(d.eventsBySeverity)
          ? d.eventsBySeverity.reduce(
        ...
        setStats({
          totalEvents,
          activeSessions: d.activeSessionsCount ?? 0,
          lockedAccounts: d.lockedAccountsCount ?? 0,
          failedLogins24h: d.failedLoginsLast24h ?? 0,
        });
  ...
  const quickLinks = [
    {
      ...
      desc: 'Manage security policy configurations',
    },
    {
      label: 'User Security Profiles',
      href: '/security/user-profiles',
      desc: 'View 2FA, risk levels, and login history',
    },
    {
      label: 'Security Events',
      href: '/security/events',
      desc: 'Review and resolve security events',
    },
    {
      label: 'Active Sessions',
      href: '/security/sessions',
      desc: 'Monitor and revoke user sessions',
    },
    {
      label: 'Two-Factor Auth',
      href: '/security/two-factor',
      desc: 'Manage 2FA adoption and requirements',
    },
  ];
    ...
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">
          Security overview — access control and audit are enforced by the backend
        </p>
      </div>
                  ...
                  {events.map((ev) => (
                    <tr key={ev.id} className="border-t border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2">
                        <span
                          className={`px-2 py-0.5 rounded text-xs font-medium ${SEVERITY_COLORS[ev.severity] ?? 'bg-gray-100 text-gray-600'}`}
                        >
                          {ev.severity}
                        </span>
                      </td>
