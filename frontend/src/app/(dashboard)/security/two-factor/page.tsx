...
export default function TwoFactorAuthPage() {
  ...
  useEffect(() => {
    fetch('/api/backend/user-security-profiles')
      .then(r => r.json())
      .then(res => setProfiles(unwrapList(res)))
      .catch(() => {})
  ...
  const statCards = [
    ...
    { label: '2FA Disabled', value: total - enabled, color: 'bg-red-50 text-red-700 border-red-200' },
    { label: 'Adoption Rate', value: `${pct}%`, color: 'bg-purple-50 text-purple-700 border-purple-200' },
  ];
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 mb-6">
            ...
            <p className="text-sm text-gray-500 mt-2">{enabled} of {total} users have 2FA enabled</p>
          </div>

          <div className="bg-blue-50 border border-blue-200 rounded-xl p-6">
            <h2 className="text-lg font-semibold text-blue-900 mb-3">Admin Guidance</h2>
            <ul className="list-disc list-inside space-y-2 text-sm text-blue-800">
              <li>Navigate to <strong>User Security Profiles</strong> to see which users have 2FA disabled</li>
              <li>Use <strong>Security Policies</strong> to enforce 2FA requirements system-wide</li>
