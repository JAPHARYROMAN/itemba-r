...
export default function LaborRecordsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/backend/itemba/labor-records?companyId=${companyId}&page=1&limit=20`);
  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <PageHeader title="Labor Records" subtitle="Worker labor and payment records" />
            ...
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  ...
                  <th className={thCls}>Worker Name</th>
                  <th className={thCls}>Role</th>
                  <th className={thCls}>Labor Date</th>
              ...
              <tbody>
                {data.data.length === 0 ? (
                  <tr><td colSpan={7} className="px-4 py-8 text-center text-sm text-slate-400">No labor records found.</td></tr>
                ) : data.data.map((l: any) => (
                  <tr key={l.id} className="border-b border-slate-50 hover:bg-slate-50">
                    <td className={`${tdCls} font-medium`}>{l.laborRecordNumber}</td>
                    <td className={tdCls}>{l.workerName ?? '—'}</td>
                    <td className={tdCls}>{l.role ?? '—'}</td>
                    <td className={tdCls}>{l.laborDate ? fmtDate(l.laborDate) : '—'}</td>
                    <td className={tdCls}>{l.hoursWorked ?? '—'}</td>
                    <td className={tdCls}>{l.totalAmount != null ? fmtCurrency(l.totalAmount) : '—'}</td>
                    <td className={tdCls}><Badge status={l.paymentStatus ?? 'PENDING'} /></td>
                  </tr>
                ))}
              </tbody>
            </table>
