...
export default function DriversPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/backend/logistics/drivers?companyId=${companyId}&page=1&limit=50`);
      if (!res.ok) throw new Error(`Failed to load drivers (HTTP ${res.status})`);
  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <PageHeader title="Drivers" subtitle="Driver registry — licensed drivers on the fleet" />
      ...
      {loading ? <Spinner /> : (
        <Card className="overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 text-xs text-slate-500">{total} drivers</div>
          <table className="w-full">
            <thead className="bg-slate-50">
              <tr>
                <th className={thCls} style={{ color: 'var(--aurora-text-secondary)' }}>Driver #</th>
                <th className={thCls} style={{ color: 'var(--aurora-text-secondary)' }}>Full Name</th>
                <th className={thCls} style={{ color: 'var(--aurora-text-secondary)' }}>License #</th>
                <th className={thCls} style={{ color: 'var(--aurora-text-secondary)' }}>Class</th>
                <th className={thCls} style={{ color: 'var(--aurora-text-secondary)' }}>Phone</th>
                <th className={thCls} style={{ color: 'var(--aurora-text-secondary)' }}>Status</th>
              </tr>
            </thead>
            <tbody>
              {items.length === 0 ? (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-400 text-sm">{companyId ? 'No drivers found.' : 'Select a company to view drivers.'}</td></tr>
              ) : items.map(d => (
                <tr key={d.id} className="border-t border-slate-100 hover:bg-slate-50">
                  <td className={tdCls}><span className="font-mono text-xs" style={{ color: 'var(--aurora-text)' }}>{d.driverNumber}</span></td>
                  ...
                  <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>{d.phoneNumber ?? '—'}</td>
