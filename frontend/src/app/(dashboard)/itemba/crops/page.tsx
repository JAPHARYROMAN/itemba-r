  ...
  return (
    <div className="p-6 space-y-5">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <PageHeader title="Crops" subtitle="Crop catalogue — registered crop types and varieties" />
      ...
      {loading ? <Spinner /> : (
        <Card className="overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 text-xs text-slate-500">{total} crops</div>
          <table className="w-full">
            <thead className="bg-slate-50">
              <tr>
                <th className={thCls} style={{ color: 'var(--aurora-text-secondary)' }}>Crop Code</th>
                ...
                <th className={thCls} style={{ color: 'var(--aurora-text-secondary)' }}>Variety</th>
                <th className={thCls} style={{ color: 'var(--aurora-text-secondary)' }}>Growing Period</th>
                <th className={thCls} style={{ color: 'var(--aurora-text-secondary)' }}>Status</th>
              </tr>
            </thead>
            <tbody>
              {items.length === 0 ? (
                <tr><td colSpan={6} className="px-4 py-8 text-center text-slate-400 text-sm">{companyId ? 'No crops found.' : 'Select a company to view crops.'}</td></tr>
              ) : items.map(c => (
                <tr key={c.id} className="border-t border-slate-100 hover:bg-slate-50">
                  <td className={tdCls}><span className="font-mono text-xs" style={{ color: 'var(--aurora-text)' }}>{c.cropCode}</span></td>
                  <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>{c.name}</td>
                  <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>{c.cropType?.replace(/_/g, ' ') ?? '—'}</td>
                  <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>{c.variety ?? '—'}</td>
                  <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>{c.growingPeriodDays != null ? `${c.growingPeriodDays} days` : '—'}</td>
                  <td className={tdCls}><Badge status={c.status} /></td>
                </tr>
              ))}
            </tbody>
          </table>
