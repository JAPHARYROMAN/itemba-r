...
export default function GRNsPage() {
  ...
  useEffect(() => {
    fetch('/api/backend/goods-received-notes')
      .then(r => r.json())
            ...
            <tbody>
              {data.length === 0 ? (
                ...
                <tr key={row.id} className="border-t border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-400">{row.receivedDate ? new Date(row.receivedDate).toLocaleDateString() : '—'}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${row.status === 'POSTED' ? 'bg-green-100 text-green-700' : row.status === 'PENDING' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600'}`}>
                      {row.status}
                    </span>
                  </td>
