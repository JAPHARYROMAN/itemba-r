...
export default function AutomationRulesPage() {
  ...
  useEffect(() => {
    fetch('/api/backend/automation-rules')
      .then(r => r.json())
  ...
  return (
    <div className="p-6">
      <div className="mb-6">
        ...
        <p className="text-gray-500 mt-1">Configure business automation rules and triggers</p>
      </div>
            ...
            <tbody>
              {data.length === 0 ? (
                ...
                <tr key={row.id} className="border-t border-gray-100 hover:bg-gray-50">
                  <td className="px-4 py-3 font-mono text-xs">{row.automationRuleCode}</td>
                  <td className="px-4 py-3 font-medium">{row.name}</td>
                  <td className="px-4 py-3">{row.automationType}</td>
                  <td className="px-4 py-3">{row.triggerType}</td>
                  <td className="px-4 py-3">
                    <span className={`px-2 py-0.5 rounded text-xs font-medium ${row.status === 'ACTIVE' ? 'bg-green-100 text-green-700' : row.status === 'PAUSED' ? 'bg-yellow-100 text-yellow-700' : 'bg-gray-100 text-gray-600'}`}>
                      {row.status}
                    </span>
                  </td>
