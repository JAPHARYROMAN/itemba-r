...
export default function RetentionOverviewPage() {
  const [stats, setStats] = useState({
    activePolicies: 0,
    legalHoldRecords: 0,
    pendingArchiveJobs: 0,
  });
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    Promise.all([
      backendList<any>('retention-policies').catch(() => []),
      backendList<any>('data-archive-jobs').catch(() => []),
    ])
      .then(([policyList, jobList]) => {
        setStats({
          activePolicies: policyList.filter((p: any) => p.status === 'ACTIVE').length,
          legalHoldRecords: policyList.filter((p: any) => p.legalHold).length,
          pendingArchiveJobs: jobList.filter((j: any) =>
            ['REQUESTED', 'APPROVED', 'RUNNING'].includes(j.status),
          ).length,
        });
      })
  ...
  const statCards = [
    ...
    {
      label: 'Legal Hold Records',
      value: stats.legalHoldRecords,
      color: 'bg-purple-50 text-purple-700 border-purple-200',
    },
    {
      label: 'Pending Archive Jobs',
      value: stats.pendingArchiveJobs,
  ...
  const links = [
    {
      ...
      desc: 'Configure data retention rules and timelines',
    },
    {
      label: 'Archive Jobs',
      href: '/retention/archive-jobs',
      desc: 'View archive and deletion job runs',
    },
  ];
