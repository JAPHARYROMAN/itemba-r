...
interface EmployeeDeduction {
  ...
  employeeId?: string;
  companyId?: string;
  deductionType?: string | { name?: string; code?: string };
...
export default function EmployeeDeductionsPage() {
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    fetch(`/api/backend/hr/deduction-types?companyId=${form.companyId}&limit=200`)
      .then(r => r.json())
      .then(j => {
        if (cancelled) return;
  ...
  const openEdit = (d: EmployeeDeduction) => {
    ...
    setForm({
      ...
      effectiveFrom: d.effectiveFrom ? d.effectiveFrom.slice(0, 10) : '',
      effectiveTo: d.effectiveTo ? d.effectiveTo.slice(0, 10) : '',
      status: d.status,
    });
    setShowModal(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        showToast('error', 'Save failed', Array.isArray(j.message) ? j.message.join(', ') : (j.message ?? 'Could not save the deduction.'));
        return;
      }
      setShowModal(false);
      load();
  ...
  const handleDelete = async () => {
    if (!deleteId) return;
    const res = await fetch(`/api/backend/hr/employee-deductions/${deleteId}`, { method: 'DELETE' });
    if (!res.ok) {
      const j = await res.json().catch(() => ({}));
      showToast('error', 'Delete failed', Array.isArray(j.message) ? j.message.join(', ') : (j.message ?? 'Could not delete the deduction.'));
    }
    setDeleteId(null);
    load();
  };
    ...
    <div className="p-6">
              ...
              <tbody style={{ color: 'var(--aurora-text)' }}>
                {rows.map(d => (
                  <tr key={d.id} className="border-b border-slate-50 hover:bg-slate-50">
                    ...
                    <td className={tdCls}>
                      {typeof d.deductionType === 'string'
                        ? d.deductionType
                        : (d.deductionType?.name ?? d.deductionType?.code ?? d.deductionTypeId ?? '—')}
                    </td>
      ...
      >
        <form id="employee-deduction-form" onSubmit={handleSubmit} className="space-y-3">
          ...
          <FormSelect label="Deduction Type" required value={form.deductionTypeId} onChange={f('deductionTypeId')}
            options={typeOptions} placeholder={form.companyId ? 'Select deduction type' : 'Select company first'} />
