...
import { LoanPaymentModal, LoanFinancialHistory } from '@/features/loans/loan-finance';
import {
...
interface Repayment {
  financialEvent?: { fees: string; reversedAt: string | null } | null;
  id: string;
  amount: string;
  principal?: string;
  interest?: string;
...
const OBLIGATION_LABELS: Record<string, string> = {
  ...
  INTER_COMPANY_LOAN: 'Inter-Company Loan',
  INSTITUTIONAL_DEBT: 'Institutional Debt',
  OTHER: 'Other',
};
...
function StatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex px-2.5 py-0.5 rounded text-xs font-semibold ${map[status] ?? 'bg-slate-100 text-slate-600'}`}
    >
      {status.replace(/_/g, ' ')}
    </span>
  );
...
function RiskBadge({ level }: { level: string }) {
  ...
  return (
    <span
      className={`inline-flex px-2.5 py-0.5 rounded text-xs ${map[level] ?? 'bg-slate-100 text-slate-600'}`}
    >
      {level}
    </span>
  );
...
function LoanPrintDocument({ loan, audit }: { loan: LoanDetail; audit: AuditEntry[] }) {
  ...
  const paidPercent =
    Number(loan.principalAmount) > 0 ? (principalPaid / Number(loan.principalAmount)) * 100 : 0;
    ...
    <div className="hidden print:block">
      ...
      >
        <DocumentSection title="Financial Summary">
          <DocumentStatGrid
            items={[
              ...
              {
                label: 'Outstanding',
                value: loanMoney(loan, loan.outstandingBalance),
                tone: Number(loan.outstandingBalance) > 0 ? 'warning' : 'success',
              },
              {
                label: 'Principal Paid',
                value: loanMoney(loan, principalPaid),
                tone: principalPaid > 0 ? 'success' : 'neutral',
              },
            ...
            items={[
              {
                ...
                value: OBLIGATION_LABELS[loan.obligationType] ?? loan.obligationType,
              },
              ...
              {
                label: 'Interest Rate',
                value: `${(parseFloat(loan.interestRate) * 100).toFixed(2)}% p.a.`,
              },
              {
                label: 'Repayment Frequency',
                value: loan.repaymentFrequency?.replace(/_/g, ' ') ?? 'N/A',
              },
              {
                label: 'Scheduled Repayment',
                value: loan.repaymentAmount ? loanMoney(loan, loan.repaymentAmount) : 'N/A',
              },
              { label: 'Start Date', value: fmtDate(loan.disbursementDate || loan.startDate) },
        ...
        <DocumentSection
          ...
          description={`${repayments.length} recorded repayment(s)`}
        >
          {repayments.length === 0 ? (
              ...
              <tbody>
                {repayments.map((repayment) => (
                  <tr key={repayment.id}>
                    ...
                    <DocumentTd align="right" mono>
                      {repayment.principal ? loanMoney(loan, repayment.principal) : 'N/A'}
                    </DocumentTd>
                    <DocumentTd align="right" mono>
              ...
              <tbody>
                {audit.map((entry) => (
                  <tr key={entry.id}>
                    ...
                    <DocumentTd>
                      {entry.user ? `${entry.user.fullName} (${entry.user.email})` : 'N/A'}
                    </DocumentTd>
...
export default function LoanDetailPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoadError('');
    try {
      const [loanRes, auditRes] = await Promise.all([
        fetch(`/api/backend/loans/${id}`, { signal: request.signal }),
        fetch(`/api/backend/loans/${id}/audit-history`, { signal: request.signal }),
      ]);
      if (!request.current()) return;
      const [loanJson, auditJson] = await Promise.all([loanRes.json(), auditRes.json()]);
      if (!request.current()) return;
      if (!loanRes.ok) throw new Error(loanJson.message ?? `Error ${loanRes.status}`);
      setLoan(loanJson.data ?? null);
      setAudit(Array.isArray(auditJson.data) ? auditJson.data : []);
    } catch (err) {
      if (!request.current()) return;
      setLoadError(err instanceof Error ? err.message : 'Failed to load loan');
      setLoan(null);
    } finally {
  ...
  const pctPaid = loan
    ? (1 - Number(loan.outstandingBalance) / Number(loan.principalAmount)) * 100
    : 0;
    ...
    return (
      <div className="min-h-screen bg-slate-50">
        <div className="max-w-screen-xl mx-auto px-6 py-8">
          <PageHeader title="Loan" subtitle="Loading" />
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-slate-50">
        <div className="flex flex-col items-center justify-center py-24 gap-4">
          <p className="text-slate-500">{loadError || 'Loan record not found.'}</p>
          {loadError && (
            <button
              type="button"
              onClick={() => void load()}
      ...
      <div className="document-no-print min-h-screen bg-slate-50">
        <div className="max-w-screen-xl mx-auto px-6 py-8 space-y-6">
          ...
          <div className="text-sm text-slate-500 flex items-center gap-2">
            <Link href="/group-control" className="hover:underline">
              Group Control
            </Link>
            <span>/</span>
            <Link href="/group-control/loans-debts" className="hover:underline">
              Loans & Debts
            </Link>
            <span>/</span>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <Card className="p-4">
              ...
              <div className="text-xl font-bold text-slate-900">
                {loan.currency} {fmt(loan.principalAmount)}
              </div>
            </Card>
            <Card className="p-4">
              <div className="text-xs text-slate-500 mb-1">Outstanding Balance</div>
              <div className="text-xl font-bold text-slate-900">
                {loan.currency} {fmt(loan.outstandingBalance)}
              </div>
            </Card>
            <Card className="p-4">
              <div className="text-xs text-slate-500 mb-1">Interest Rate</div>
              <div className="text-xl font-bold text-slate-900">
                {(parseFloat(loan.interestRate) * 100).toFixed(2)}% p.a.
              </div>
            </Card>
            <Card className="p-4">
              ...
              <div className="text-xs text-slate-400">
                {loan.repaymentFrequency?.replace(/_/g, ' ') ?? ''}
              </div>
            </Card>
          </div>

          <LoanFinancialHistory loanId={loan.id} onChanged={load} />
          ...
          {pctPaid > 0 && (
            <Card className="p-4">
              ...
              <div className="flex justify-between text-xs text-slate-400 mt-1">
                <span>
                  {loan.currency}{' '}
                  {fmt(Number(loan.principalAmount) - Number(loan.outstandingBalance))} paid
                </span>
                <span>
                  {loan.currency} {fmt(loan.outstandingBalance)} remaining
                </span>
              </div>
            </Card>
          )}
          ...
          <div>
            <div className="flex gap-1 border-b border-slate-200 mb-5">
              {(['details', 'repayments', 'audit'] as const).map((t) => (
                ...
                >
                  {t === 'repayments'
                    ? `Repayment History (${loan.repayments?.length ?? 0})`
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <Card className="p-5">
                  ...
                  <dl>
                    ...
                    <DetailRow
                      label="Start Date"
                      value={fmtDate(loan.disbursementDate || loan.startDate)}
                    />
                    <DetailRow label="Maturity Date" value={fmtDate(loan.maturityDate)} />
                    <DetailRow
                      label="Repayment Frequency"
                      value={loan.repaymentFrequency?.replace(/_/g, ' ')}
                    />
                  </dl>
                </Card>
                ...
                <Card className="p-5">
                  ...
                  <dl>
                    ...
                    <DetailRow
                      label="Borrower"
                      value={loan.company?.name ?? loan.group?.name ?? '—'}
                    />
                  ...
                  <Card className="p-5">
                    ...
                    <dl>
                      ...
                      {(loan.linkedAssetIds?.length ?? 0) > 0 && (
                        <DetailRow
                          label="Linked Asset IDs"
                          value={
                            <div className="flex flex-wrap gap-1">
                              {loan.linkedAssetIds!.map((aid) => (
                                <Link
                                  key={aid}
                                  href={`/group-control/fixed-assets/${aid}`}
                                  className="font-mono text-xs bg-slate-100 px-2 py-0.5 rounded hover:bg-brand-50 hover:text-brand-700"
                                >
                                  {aid.slice(0, 8)}…
                                </Link>
                              ))}
                  ...
                  <WorkspaceTable className="w-full text-sm">
                    <thead className="text-left text-slate-500 border-b border-slate-200 bg-slate-50">
                      <tr>
                        ...
                        <th className="px-5 py-2">Fees</th>
                      ...
                      {loan.repayments.map((r) => (
                        <tr key={r.id} className="hover:bg-slate-50">
                          <td className="px-5 py-2 text-slate-700">
                            {fmtDate(r.repaymentDate)}
                            {r.financialEvent?.reversedAt ? ' · Reversed' : ''}
                          </td>
                          <td className="px-5 py-2 font-semibold text-slate-900">
                            {loan.currency} {fmt(r.amount)}
                          </td>
                          <td className="px-5 py-2 text-slate-600">
                            {r.principal ? fmt(r.principal) : '—'}
                          </td>
                          <td className="px-5 py-2 text-slate-600">
                            {r.interest ? fmt(r.interest) : '—'}
                          </td>
                          <td className="px-5 py-2 text-slate-600">
                            {r.financialEvent?.fees ? fmt(r.financialEvent.fees) : '—'}
                          </td>
                          <td className="px-5 py-2 text-slate-600">
                            {r.penalties && parseFloat(r.penalties) > 0 ? fmt(r.penalties) : '—'}
                          </td>
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {audit.map((a) => (
                        <tr key={a.id} className="hover:bg-slate-50">
                          <td className="px-5 py-2 text-slate-500 text-xs whitespace-nowrap">
