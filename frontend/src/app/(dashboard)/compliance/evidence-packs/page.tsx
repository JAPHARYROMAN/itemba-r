...
export default function EvidencePacksPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/backend/audit-evidence-packs?${params}`, { signal: request.signal });
      if (!request.current()) return;
      if (!res.ok) throw new Error('Failed to load evidence packs');
      ...
      if (!request.current()) return;
      setLoadError(err instanceof Error ? err.message : 'Failed to load evidence packs');
      setItems([]);
      setTotal(0);
      setTotalPages(1);
    } finally {
  ...
  }, [authLoading, beginRequest, canView, page, companyId, packType, status]);
  ...
  if (authLoading) return <div className="p-6"><PageHeader title="Evidence Packs" subtitle="Loading" /></div>;
      ...
      <PageToolbar
        filters={
          <>
            ...
            <select aria-label="All Types" value={packType} onChange={(e) => reset(setPackType)(e.target.value)} className={filterSelectCls} style={filterStyle}>
