...
const BLANK_FORM: CashAccountForm = {
  companyId: '', accountName: '', accountType: 'CASH_ON_HAND', currency: 'TZS',
  openingBalance: 0, linkedBankAccountId: '', notes: '', isActive: true,
};
...
function CashAccountModal({ mode, initial, companies, onClose, onSaved }: {
  mode: 'create' | 'edit'; initial?: CashAccount; companies: Company[]; onClose: () => void; onSaved: () => void;
}) {
  const [form, setForm] = useState<CashAccountForm>(() => initial ? {
    companyId: initial.companyId, accountName: initial.accountName, accountType: initial.accountType,
    currency: initial.currency, openingBalance: initial.openingBalance,
    linkedBankAccountId: initial.linkedBankAccountId ?? '', notes: initial.notes ?? '', isActive: initial.isActive,
  } : { ...BLANK_FORM });
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState('');

  const set = <K extends keyof CashAccountForm>(k: K, v: CashAccountForm[K]) => setForm((f) => ({ ...f, [k]: v }));

  const handleSubmit = async () => {
    if (!form.companyId || !form.accountName.trim()) { setError('Company and account name required'); return; }
    setSaving(true); setError('');
    try {
      const body = { ...form, openingBalance: Number(form.openingBalance) || 0,
        linkedBankAccountId: form.linkedBankAccountId || undefined, notes: form.notes || undefined };
      const res = await fetch(
        mode === 'create' ? '/api/backend/cash-accounts' : `/api/backend/cash-accounts/${initial!.id}`,
  ...
  return (
    <Modal open onClose={onClose} title={mode === 'create' ? 'Create Cash Account' : 'Edit Cash Account'} size="lg"
      ...
      <div className="space-y-3">
        ...
        <FormInput label="Account Name" required value={form.accountName} onChange={(e) => set('accountName', e.target.value)} placeholder="e.g. Main Cash Box" />
        ...
        <FormInput label="Opening Balance" type="number" step="0.01" value={form.openingBalance} onChange={(e) => set('openingBalance', e.target.value === '' ? '' : Number(e.target.value))} disabled={mode === 'edit'} />
        <FormInput label="Linked Bank Account ID (optional)" value={form.linkedBankAccountId} onChange={(e) => set('linkedBankAccountId', e.target.value)} />
...
function DeleteConfirm({ acc, onClose, onDeleted }: { acc: CashAccount; onClose: () => void; onDeleted: () => void }) {
  const [deleting, setDeleting] = useState(false);
  const confirm = async () => { setDeleting(true); await fetch(`/api/backend/cash-accounts/${acc.id}`, { method: 'DELETE' }); onDeleted(); };
  return (
    <Modal open onClose={onClose} title="Delete Cash Account" size="md"
      footer={<><Btn variant="secondary" onClick={onClose}>Cancel</Btn><Btn variant="danger" onClick={confirm} loading={deleting}>Delete</Btn></>}>
      <p className="text-sm" style={{ color: 'var(--aurora-text)' }}>Delete cash account <span className="font-medium">{acc.accountName}</span>?</p>
    </Modal>
  );
}
...
export default function CashAccountsPage() {
  ...
  }, [canView, companyId, accountType, activeFilter]);
  ...
  return (
    <div className="p-6 space-y-6">
      {creating && <CashAccountModal mode="create" companies={companies} onClose={() => setCreating(false)} onSaved={() => { setCreating(false); load(); }} />}
      {editing && <CashAccountModal mode="edit" initial={editing} companies={companies} onClose={() => setEditing(null)} onSaved={() => { setEditing(null); load(); }} />}
      {deleting && <DeleteConfirm acc={deleting} onClose={() => setDeleting(null)} onDeleted={() => { setDeleting(null); load(); }} />}

      <PageHeader title="Cash Accounts" subtitle="Manage cash, bank, and mobile money accounts" />
      ...
      <PageToolbar
        filters={
          <>
            ...
            <select value={accountType} onChange={(e) => setAccountType(e.target.value)} className={filterSelectCls} style={filterStyle}>
              <option value="">All Types</option>
              {ACCOUNT_TYPES.map((t) => <option key={t} value={t}>{t.replace(/_/g, ' ')}</option>)}
            </select>
            <select value={activeFilter} onChange={(e) => setActiveFilter(e.target.value)} className={filterSelectCls} style={filterStyle}>
          ...
          <table className="w-full text-sm min-w-[800px]">
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                <tr><td colSpan={6} className="px-4 py-10 text-center text-sm" style={{ color: 'var(--aurora-text-muted)' }}>No cash accounts found</td></tr>
              ) : list.map((acc) => (
                <tr key={acc.id} className="hover:bg-slate-50">
                  <td className="px-4 py-3 font-medium">{acc.accountName}</td>
                  <td className="px-4 py-3">{acc.company?.name ?? '—'}</td>
                  <td className="px-4 py-3">
                    <span className={`inline-flex items-center px-2 py-0.5 rounded text-xs border ${TYPE_BADGE[acc.accountType] ?? TYPE_BADGE.OTHER}`}>
                      {acc.accountType.replace(/_/g, ' ')}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right font-mono">{fmtMoney(acc.currentBalance, acc.currency)}</td>
                  <td className="px-4 py-3">
