...
import { Btn, Card, DetailDrawer, FormDateField, FormSelect, FormTextarea, Modal, PageHeader, PageSpinner, PageToolbar, StatCard, StatusBadge } from '@/components/ui';
...
function ReverseDialog({
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(j.message ?? 'Reverse failed');
      }
      onDone();
    } catch (err: unknown) {
  ...
  return (
    ...
    >
      ...
      <p className="mb-3 text-sm" style={{ color: 'var(--aurora-text)' }}>
        This will create a reversing entry for {entry.journalNumber ?? entry.id.slice(0, 8)}.
      </p>
      <FormTextarea
...
function JournalDrawer({ entry, onClose }: { entry: JournalEntry; onClose: () => void }) {
  return (
    <DetailDrawer
      open
      onClose={onClose}
      title={entry.journalNumber ?? 'Journal entry'}
      subtitle={formatDate(entry.transactionDate)}
      width="lg"
    >
      <div className="p-5 space-y-4">
        <div className="grid grid-cols-2 gap-3 text-sm">
          <div>
            <span className="text-xs uppercase" style={{ color: 'var(--aurora-text-muted)' }}>
              Company
            </span>
          ...
          <div>
            ...
            <div>{entry.accountingPeriod?.name ?? '—'}</div>
          </div>
          <div>
            <span className="text-xs uppercase" style={{ color: 'var(--aurora-text-muted)' }}>
              Status
...
function JournalModal({
  ...
  useEffect(() => {
    if (companyId) {
      fetch(`/api/backend/accounting-periods?companyId=${companyId}&status=OPEN`)
        .then((r) => r.json())
        .then((j) =>
          setPeriods(
      ...
      fetch(`/api/backend/chart-of-accounts?companyId=${companyId}&limit=200&isActive=true`)
        .then((r) => r.json())
        .then((j) =>
          setAccounts(
            Array.isArray(j.data?.data) ? j.data.data : Array.isArray(j.data) ? j.data : [],
  ...
  const handleSubmit = async () => {
    if (!companyId || !transactionDate || !description.trim()) {
    ...
    if (lines.some((l) => !l.accountId)) {
      setError('All lines must have an account');
      return;
    }
    setSaving(true);
    setError('');
    try {
      ...
      const res = await fetch(
        mode === 'create'
          ? '/api/backend/journal-entries'
          : `/api/backend/journal-entries/${initial!.id}`,
        {
    ...
    <Modal
      ...
      footer={
        <>
          ...
          <Btn variant="primary" onClick={handleSubmit} loading={saving} disabled={!balanced}>
      ...
      <div className="space-y-3">
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <h4 className="text-sm font-medium" style={{ color: 'var(--aurora-text)' }}>
              Lines
            </h4>
            <Btn variant="ghost" size="xs" onClick={addLine}>
              + Add Line
            </Btn>
          </div>
          <div
                  ...
                  <tr key={i}>
                    ...
                    <td className="px-2 py-1">
                      <input
                        ...
                        className="w-full text-xs border rounded px-1 py-1 text-right font-mono"
                        style={{
                          borderColor: 'var(--aurora-border)',
                          background: 'var(--aurora-card)',
                          color: 'var(--aurora-text)',
          ...
          <div
            className={`mt-2 text-xs font-medium ${balanced ? 'text-emerald-600' : 'text-red-600'}`}
          >
...
function DeleteConfirm({
  ...
  const confirm = async () => {
    setDeleting(true);
    await fetch(`/api/backend/journal-entries/${entry.id}`, { method: 'DELETE' });
    onDeleted();
  };
  return (
    <Modal
    ...
    >
      <p className="text-sm" style={{ color: 'var(--aurora-text)' }}>
        Delete entry{' '}
        <span className="font-medium">{entry.journalNumber ?? entry.id.slice(0, 8)}</span>?
      </p>
    </Modal>
  );
}
...
export default function JournalEntriesPage() {
  ...
  useEffect(() => {
    ...
    if (companyId) {
      const controller = new AbortController();
      fetch(`/api/backend/accounting-periods?companyId=${companyId}`, { signal: controller.signal })
        .then((r) => r.json())
        .then((j) => {
          if (controller.signal.aborted) return;
          setFilterPeriods(
        ...
        .catch(() => {
          if (!controller.signal.aborted) setFilterPeriods([]);
        });
      return () => controller.abort();
    } else {
      setFilterPeriods([]);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/backend/journal-entries?${params}`, { signal: request.signal });
      const json = await res.json();
      if (!request.current()) return;
      if (!res.ok) throw new Error(json.message ?? 'Unable to load journal entries');
      setData(json.data ?? null);
    } catch (err) {
      if (!request.current()) return;
      setLoadError(err instanceof Error ? err.message : 'Unable to load journal entries');
      setData(null);
    } finally {
      if (request.current()) setLoading(false);
    }
  }, [authLoading, beginRequest, canView, page, companyId, status, accountingPeriodId, dateFrom, dateTo]);
  ...
  const handleAction = async (id: string, action: string) => {
    ...
    try {
      const res = await fetch(`/api/backend/journal-entries/${id}/${action}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: '{}',
      });
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <PageHeader
        ...
        subtitle="Manual ledger entries with debits and credits"
      />
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <StatCard
          label="Posted"
          value={data?.data.filter((e) => e.status === 'POSTED').length ?? 0}
        />
        <StatCard
          label="Reversed"
          value={data?.data.filter((e) => e.status === 'REVERSED').length ?? 0}
        />
      </div>
      ...
      <PageToolbar
        collapsibleFilters
        activeFilterCount={
          [companyId, accountingPeriodId, status, dateFrom, dateTo].filter(Boolean).length
        }
        filters={
          <>
            <FormSelect
            ...
            <FormSelect
              ...
              disabled={!companyId}
              onChange={(e) => {
                setAccountingPeriodId(e.target.value);
                setPage(1);
              }}
            >
                ...
                data.data.map((e) => (
                  ...
                  >
                    <td className="px-4 py-3 font-mono text-xs">
                      {e.journalNumber ?? e.id.slice(0, 8)}
                    </td>
                          ...
                          <>
                            {canPost && (
                              <Btn
                                ...
                                loading={actionLoading === e.id + 'post'}
                              >
                                Post
                              </Btn>
                            )}
                            <Btn variant="ghost" size="xs" onClick={() => setEditing(e)}>
                              Edit
                            </Btn>
                            <Btn variant="danger" size="xs" onClick={() => setDeleting(e)}>
                              Delete
                            </Btn>
                          </>
                        )}
                        {e.status === 'POSTED' && canReverse && (
                          <Btn variant="warning" size="xs" onClick={() => setReversing(e)}>
                            Reverse
                          </Btn>
                        )}
                      </div>
