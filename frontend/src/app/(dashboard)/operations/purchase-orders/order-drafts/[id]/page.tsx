...
function Field({ label, value }: { label: string; value?: React.ReactNode }) {
  return (
    <div>
      ...
      <div className="mt-1 break-words text-[13px]">{value || '-'}</div>
    </div>
  );
}
...
export default function SupplierOrderDraftDetailPage() {
  ...
  if (loading)
    return (
      <div className="space-y-5 p-6">
        <PageHeader title="Supplier Order Draft" subtitle="Loading planning document" />
  ...
  if (!draft)
    return (
      <div className="p-6">
        <PageHeader title="Supplier Order Draft" subtitle={error || 'Document not found'} />
        <Link
          href="/operations/purchase-orders/order-drafts"
          className="mt-5 inline-block text-sm text-brand-600"
        >
          Back to drafts
        </Link>
      </div>
    ...
    <div className="space-y-5 p-6">
      {editing && (
        <SupplierOrderDraftForm
          open
          companies={companies.length ? companies : [draft.company!]}
          initial={draft}
      ...
      <ConfirmDialog
        ...
        title={`${pending ? pending[0].toUpperCase() + pending.slice(1) : ''} supplier order draft`}
        message={`Confirm that you want to ${pending} ${draft.draftNumber}. This changes only the planning document lifecycle.`}
        confirmLabel={pending === 'send' ? 'Mark Sent' : 'Confirm'}
        variant={pending === 'decline' || pending === 'cancel' ? 'danger' : 'default'}
        loading={acting}
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {hasPermission('supplier_order_drafts.create') && (
            <Btn
              size="sm"
              variant="secondary"
          ...
          <Link href={`/operations/purchase-orders/order-drafts/${draft.id}/print`}>
            <Btn size="sm" variant="secondary" icon={<ExternalLink className="h-3.5 w-3.5" />}>
              Print View
            </Btn>
          </Link>
          {hasPermission('supplier_order_drafts.export') && (
            <DocumentArtifactButton entityType="SUPPLIER_ORDER_DRAFT" entityId={draft.id} compact />
          )}
          {hasPermission('supplier_order_drafts.export') && (
            <Btn
              size="sm"
          ...
          {draft.status === 'DRAFT' && canManage && (
            <Btn size="sm" variant="warning" onClick={() => setPending('cancel')}>
              Cancel
            </Btn>
          )}
          {draft.status === 'SENT' && canManage && (
            <>
              <Btn size="sm" variant="success" onClick={() => setPending('accept')}>
                Accept
              </Btn>
              <Btn size="sm" variant="danger" onClick={() => setPending('decline')}>
                Decline
              </Btn>
              <Btn size="sm" variant="warning" onClick={() => setPending('cancel')}>
                Cancel
          ...
          {draft.status !== 'DRAFT' && canManage && (
            <Btn size="sm" variant="secondary" onClick={() => setPending('reopen')}>
              Reopen
            </Btn>
          )}
        </div>
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <span className="text-xs" style={{ color: 'var(--aurora-text-muted)' }}>
          Created {dateOnly(draft.createdAt)} by {draft.createdBy?.fullName || 'System user'}
        </span>
      </div>

      <div className="grid gap-4 xl:grid-cols-3">
        <Card>
          <h2 className="mb-4 text-sm font-semibold">Document Overview</h2>
          <div className="grid grid-cols-2 gap-4">
            <Field label="Draft Date" value={dateOnly(draft.draftDate)} />
            <Field label="Needed By" value={dateOnly(draft.neededBy)} />
            <Field label="Currency" value={draft.currency} />
            <Field label="Title" value={draft.title} />
            <Field label="Company" value={draft.company?.name} />
            <Field label="Division" value={draft.division?.name} />
            <Field label="Branch" value={draft.branch?.name} />
            <Field label="Updated" value={dateOnly(draft.updatedAt)} />
          </div>
        </Card>
        <Card>
          <h2 className="mb-4 text-sm font-semibold">Frozen Supplier Details</h2>
          <div className="grid grid-cols-2 gap-4">
            <Field label="Supplier" value={draft.supplierName} />
            <Field
              label="Source"
              value={draft.supplierId ? 'Saved supplier snapshot' : 'One-off supplier'}
            />
            <Field label="Contact" value={draft.supplierContact} />
            <Field label="Phone" value={draft.supplierPhone} />
            <Field label="Email" value={draft.supplierEmail} />
            <Field label="TIN" value={draft.supplierTin} />
            <Field label="VRN" value={draft.supplierVrn} />
            <Field label="Address" value={draft.supplierAddress} />
          </div>
        </Card>
        <Card>
          <h2 className="mb-4 text-sm font-semibold">Pricing Summary</h2>
          <div className="space-y-3 text-sm">
            ...
            >
              ...
              <b className="text-base">{money(draft.totalAmount, draft.currency)}</b>
            </div>
          </div>
          {draft.hasUnpricedLines && (
            <p className="mt-4 rounded-md bg-amber-50 px-3 py-2 text-xs text-amber-800">
              The total includes priced lines only. Unpriced lines remain “Price to be confirmed”.
            </p>
          )}
        </Card>
      </div>
      ...
      <Card padding="none" className="overflow-hidden">
        <div className="border-b px-5 py-4" style={{ borderColor: 'var(--aurora-border)' }}>
          ...
          <p className="text-xs" style={{ color: 'var(--aurora-text-muted)' }}>
            Manual planning lines; no inventory or product records are linked.
          </p>
        </div>
            ...
            <tbody>
              {draft.lines.map((line) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">{line.unitLabel}</td>
                  <td className="px-4 py-3 text-right">
                    {line.unitPrice === null ? (
                      <span className="text-amber-600">Price to be confirmed</span>
                    ) : (
                      money(line.unitPrice, draft.currency)
                    )}
                  </td>
                  <td className="px-4 py-3 text-right">
                    {line.unitPrice === null ? '-' : money(line.discountAmount, draft.currency)}
                  </td>
                  <td className="px-4 py-3 text-right">
                    {line.unitPrice === null ? '-' : money(line.taxAmount, draft.currency)}
                  </td>
                  <td className="px-4 py-3 text-right font-semibold">
                    {line.lineTotal === null ? (
                      <span className="text-amber-600">Pending</span>
                    ) : (
                      money(line.lineTotal, draft.currency)
                    )}
                  </td>
                </tr>
              ))}
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <Card>
          <h2 className="mb-2 text-sm font-semibold">Delivery Instructions</h2>
          <p className="whitespace-pre-wrap text-sm">{draft.deliveryInstructions || '-'}</p>
        </Card>
        <Card>
          <h2 className="mb-2 text-sm font-semibold">Terms</h2>
          <p className="whitespace-pre-wrap text-sm">{draft.terms || '-'}</p>
        </Card>
        <Card>
