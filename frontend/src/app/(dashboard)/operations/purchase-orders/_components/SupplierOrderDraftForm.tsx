...
type EditLine = SupplierOrderDraftLine & {
  key: string;
  quantity: string;
  unitPrice: string;
...
function toEditLine(line: SupplierOrderDraftLine): EditLine {
  return {
    ...
    unitPrice:
      line.unitPrice === null || line.unitPrice === undefined ? '' : String(line.unitPrice),
...
export function SupplierOrderDraftForm({ open, companies, initial, onClose, onSaved }: Props) {
  ...
  const [supplierMode, setSupplierMode] = useState<'saved' | 'manual'>(
    initial?.supplierId ? 'saved' : 'manual',
  );
  ...
  const [lines, setLines] = useState<EditLine[]>(
    initial?.lines?.length ? initial.lines.map(toEditLine) : [blankLine()],
  );
  ...
  const visibleBranches = divisionId
  ...
  async function save() {
    setError('');
    if (!companyId || !draftDate || !currency)
      return setError('Company, draft date, and currency are required.');
    if (supplierMode === 'saved' ? !supplierId : !supplierName.trim())
      return setError('Select a supplier or enter a manual supplier name.');
    if (
      !lines.length ||
      lines.some(
        (line) => !line.description.trim() || Number(line.quantity) <= 0 || !line.unitLabel.trim(),
      )
    ) {
    ...
    try {
      ...
      showToast(
        'success',
        initial ? 'Supplier order draft updated' : 'Supplier order draft created',
      );
      onSaved(result);
    } catch (cause) {
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={initial ? `Edit ${initial.draftNumber}` : 'New Supplier Order Draft'}
      subtitle="Planning document only. This will not create stock, payables, journals, or an actual purchase order."
      size="3xl"
      ...
      <div className="space-y-6">
        ...
        <section>
          <div className="mb-3 flex items-center justify-between gap-3">
            ...
            >
              <button
                type="button"
                className={`rounded-md px-3 py-1 text-xs ${supplierMode === 'saved' ? 'bg-brand-600 text-white' : ''}`}
              ...
              <button
                type="button"
                className={`rounded-md px-3 py-1 text-xs ${supplierMode === 'manual' ? 'bg-brand-600 text-white' : ''}`}
          ...
          {supplierMode === 'saved' ? (
            <FormSelect
              ...
              options={suppliers.map((supplier) => ({
                value: supplier.id,
                label: `${supplier.supplierCode ?? ''} ${supplier.name}`.trim(),
              }))}
            />
          ) : (
            <div className="grid gap-3 md:grid-cols-4">
              ...
              <FormInput
                label="TIN"
                value={supplierTin}
                onChange={(event) => setSupplierTin(event.target.value)}
              />
              <FormInput
                label="VRN"
                value={supplierVrn}
                onChange={(event) => setSupplierVrn(event.target.value)}
              />
              <FormTextarea
        ...
        <section>
          ...
          <div className="grid gap-3 md:grid-cols-4">
            ...
            <FormSelect
              ...
              options={['TZS', 'USD', 'EUR'].map((code) => ({ value: code, label: code }))}
            />
            <FormInput
        ...
        <section>
          <div className="mb-3 flex items-center justify-between">
            <div>
              <h3 className="text-sm font-semibold">Manual Line Items</h3>
              <p className="text-xs" style={{ color: 'var(--aurora-text-muted)' }}>
                No product or unit database record is required. Leave unit price blank when awaiting
                a quote.
              </p>
            </div>
            <Btn
                ...
                >
                  ...
                  <div className="grid gap-3 md:grid-cols-12">
                    <FormInput
                      ...
                      label="Item Code"
                      value={line.itemCode ?? ''}
                      onChange={(event) => patchLine(line.key, { itemCode: event.target.value })}
                    />
                    <FormInput
                    ...
                    <FormInput
                      ...
                      onChange={(event) => patchLine(line.key, { unitLabel: event.target.value })}
                    />
                    <FormInput
                      className="md:col-span-2"
                      label="Unit Price"
                      hint="Blank = confirm later"
                      type="number"
                      min="0.0001"
                      step="any"
                      ...
                      onChange={(event) =>
                        patchLine(line.key, {
                          ...
                          ...(!event.target.value && { discountAmount: '0', taxAmount: '0' }),
                        })
                      }
                    />
                    <div className="md:col-span-2">
                      ...
                      <div
                        ...
                        style={{ borderColor: 'var(--aurora-border)' }}
                      >
                        {lineTotal === null ? 'Price to be confirmed' : money(lineTotal, currency)}
                      </div>
                    </div>
                    {priced && (
                      <>
                        <FormInput
                          ...
                          onChange={(event) =>
                            patchLine(line.key, { discountAmount: event.target.value })
                          }
                        />
                        <FormInput
                          className="md:col-span-2"
                          ...
                          onChange={(event) =>
                            patchLine(line.key, { taxAmount: event.target.value })
                          }
                        />
                        <FormInput
                          className="md:col-span-4"
          ...
          <div className="rounded-lg border p-4" style={{ borderColor: 'var(--aurora-border)' }}>
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <b>{money(totals.subtotal, currency)}</b>
              </div>
              <div className="flex justify-between">
                <span>Discount</span>
                <b>-{money(totals.discount, currency)}</b>
              </div>
              <div className="flex justify-between">
                <span>Tax</span>
                <b>{money(totals.tax, currency)}</b>
              </div>
              <div
                className="flex justify-between border-t pt-2"
                style={{ borderColor: 'var(--aurora-border)' }}
              >
                <span className="font-semibold">{totals.unpriced ? 'Partial total' : 'Total'}</span>
                <b>{money(totals.total, currency)}</b>
              </div>
            </div>
            {totals.unpriced > 0 && (
              <p className="mt-3 rounded-md bg-amber-50 px-2 py-1.5 text-xs text-amber-800">
                {totals.unpriced} line{totals.unpriced === 1 ? '' : 's'} marked “Price to be
                confirmed”.
              </p>
            )}
          </div>
