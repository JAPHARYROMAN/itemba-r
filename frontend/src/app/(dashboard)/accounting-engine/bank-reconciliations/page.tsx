...
export default function BankReconciliationsPage() {
  ...
  const saveReconciliation = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/backend/bank-reconciliations', {
        method: 'POST',
  ...
  const saveLine = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/backend/bank-reconciliations/${selected.id}/lines`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      });
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader
        ...
        subtitle="Prepare, match, approve, and close bank reconciliation workpapers"
      />
      ...
      <Card className="p-4">
        <div className="grid md:grid-cols-[1fr_180px_auto] gap-3 items-end">
          ...
          >
            {['DRAFT', 'IN_PROGRESS', 'RECONCILED', 'APPROVED', 'CLOSED', 'CANCELLED'].map((s) => (
                ...
                <tbody>
                  {rows.length === 0 ? (
                    ...
                    rows.map((row) => (
                      <tr
                        key={row.id}
                        onClick={() => refreshSelected(row.id)}
                        className="border-t cursor-pointer hover:bg-slate-50"
                        style={{ borderColor: 'var(--aurora-border)' }}
                      >
                        <td className="px-4 py-3 font-mono text-xs">{row.reconciliationNumber}</td>
                        <td className="px-4 py-3">
                          {cashAccountById.get(row.cashAccountId)?.accountName ?? row.cashAccountId}
                        </td>
                        <td className="px-4 py-3 text-xs">
                          {row.statementStartDate?.slice(0, 10)} to{' '}
                          {row.statementEndDate?.slice(0, 10)}
                        </td>
                        <td
                          className={`px-4 py-3 text-right font-mono ${Math.abs(Number(row.differenceAmount ?? 0)) > 0.01 ? 'text-red-600' : 'text-emerald-600'}`}
                        >
                          {fmtMoney(row.differenceAmount)}
                        </td>
                        <td className="px-4 py-3">
        ...
        <Card className="p-4 space-y-4">
          {!selected ? (
            <div className="text-sm" style={{ color: 'var(--aurora-text-muted)' }}>
              Select a reconciliation to work on statement lines and matching.
            </div>
          ) : (
            <>
              <div>
                ...
                <div className="text-xs" style={{ color: 'var(--aurora-text-muted)' }}>
                  {selected.statementStartDate?.slice(0, 10)} to{' '}
                  {selected.statementEndDate?.slice(0, 10)}
                </div>
              </div>
              <div className="grid grid-cols-2 gap-2 text-sm">
                <StatCard
                  label="Statement Close"
                  value={fmtMoney(selected.statementClosingBalance)}
                />
              ...
              <div className="flex flex-wrap gap-2">
                ...
                <Btn
                  ...
                  loading={saving}
                  onClick={() =>
                    action(selected.id, 'run-matching', {
                      dateWindowDays: 3,
                ...
                <Btn
                  ...
                  variant="primary"
                  loading={saving}
                  onClick={() => action(selected.id, 'close')}
              ...
              <div
                className="max-h-[360px] overflow-auto border rounded-lg"
                style={{ borderColor: 'var(--aurora-border)' }}
              >
                {(selected.statementLines ?? []).map((line) => (
                  ...
                  >
                    ...
                    <div className="mt-1 font-mono text-xs">
                      DR {fmtMoney(line.debitAmount)} / CR {fmtMoney(line.creditAmount)}
                    </div>
                  </div>
                ))}
              </div>
        ...
        <Modal
          ...
          footer={
            <>
              ...
              <Btn loading={saving} onClick={saveReconciliation}>
                Create
              </Btn>
            </>
          }
        >
          <div className="grid md:grid-cols-2 gap-3">
            <FormInput
              ...
              onChange={(e) => setForm((f) => ({ ...f, reconciliationNumber: e.target.value }))}
            />
            <FormSelect
              label="Company"
              required
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, companyId: e.target.value, cashAccountId: '' }))
              }
            ...
            >
              {cashAccounts
                .filter((account) => account.companyId === form.companyId)
                .map((account) => (
            ...
            <FormInput
              ...
              onChange={(e) => setForm((f) => ({ ...f, statementStartDate: e.target.value }))}
            />
            <FormInput
              label="Statement End"
              type="date"
              value={form.statementEndDate}
              onChange={(e) => setForm((f) => ({ ...f, statementEndDate: e.target.value }))}
            />
            <FormInput
              label="Statement Opening"
              type="number"
              value={form.statementOpeningBalance}
              onChange={(e) => setForm((f) => ({ ...f, statementOpeningBalance: e.target.value }))}
            />
            <FormInput
            ...
            <FormInput
              label="Book Opening"
              type="number"
              value={form.bookOpeningBalance}
              onChange={(e) => setForm((f) => ({ ...f, bookOpeningBalance: e.target.value }))}
            />
            <FormInput
              label="Book Closing"
              type="number"
              value={form.bookClosingBalance}
              onChange={(e) => setForm((f) => ({ ...f, bookClosingBalance: e.target.value }))}
            />
      ...
      {lineOpen && selected && (
        <Modal
          ...
          footer={
            <>
              <Btn variant="secondary" onClick={() => setLineOpen(false)}>
                Cancel
              </Btn>
              <Btn loading={saving} onClick={saveLine}>
                Add Line
              </Btn>
            </>
          }
        >
          <div className="grid md:grid-cols-2 gap-3">
            <FormInput
              ...
              onChange={(e) => setLineForm((f) => ({ ...f, transactionDate: e.target.value }))}
            />
            <FormInput
              label="Reference"
            ...
            <FormInput
              ...
              onChange={(e) => setLineForm((f) => ({ ...f, debitAmount: e.target.value }))}
            />
            <FormInput
              label="Credit"
              type="number"
              value={lineForm.creditAmount}
              onChange={(e) => setLineForm((f) => ({ ...f, creditAmount: e.target.value }))}
            />
            <FormInput
              label="Running Balance"
              type="number"
              value={lineForm.balance}
              onChange={(e) => setLineForm((f) => ({ ...f, balance: e.target.value }))}
            />
          </div>
        </Modal>
      )}
