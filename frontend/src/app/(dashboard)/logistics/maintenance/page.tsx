...
const MAINTENANCE_TYPES = ['SERVICE','REPAIR','INSPECTION','TYRE','OIL_CHANGE','BREAKDOWN','OTHER'];
...
const EMPTY_FORM = { vehicleId: '', maintenanceType: 'SERVICE', maintenanceDate: '', odometerReading: '', description: '', costAmount: '', currency: 'TZS', nextServiceDate: '', nextServiceOdometer: '', notes: '' };
...
export default function MaintenancePage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/backend/logistics/vehicle-maintenance?companyId=${companyId}&page=1&limit=20`);
  ...
  async function handleSave() {
    ...
    try {
      ...
      const url = isEdit ? `/api/backend/logistics/vehicle-maintenance/${(modal as any).id}` : '/api/backend/logistics/vehicle-maintenance';
  ...
  async function handleDelete() {
    if (!deleteId) return;
    try {
      const res = await fetch(`/api/backend/logistics/vehicle-maintenance/${deleteId}`, {
        method: 'DELETE',
    ...
    <div className="p-6 space-y-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>Maintenance #</th>
              ...
              <tbody>
                {data.data.length === 0 ? (
                  <tr><td colSpan={8} className="px-4 py-8 text-center text-sm" style={{ color: 'var(--aurora-text-muted)' }}>No maintenance records found.</td></tr>
                ) : data.data.map((m: any) => (
                  <tr key={m.id} className="border-b border-slate-50 hover:bg-slate-50">
                    <td className={`${tdCls} font-medium`} style={{ color: 'var(--aurora-text)' }}>{m.maintenanceNumber}</td>
                    <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>{m.vehicle?.registrationNumber ?? m.vehicleId ?? '—'}</td>
                    <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>{m.maintenanceType?.replace(/_/g, ' ') ?? '—'}</td>
                    <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>{m.maintenanceDate ? fmtDate(m.maintenanceDate) : '—'}</td>
                    <td className={`${tdCls} max-w-xs truncate`} style={{ color: 'var(--aurora-text)' }}>{m.description ?? '—'}</td>
                    <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>{m.costAmount != null ? fmtCurrency(m.costAmount) : '—'}</td>
                    <td className={tdCls}><StatusBadge status={m.status} /></td>
                    <td className={tdCls}>
                      <div className="flex gap-3">
                        <Btn variant="ghost" size="xs" onClick={() => openEdit(m)}>Edit</Btn>
                        <Btn variant="danger" size="xs" onClick={() => { setDeleteId(m.id); setDeleteLabel(m.maintenanceNumber || m.id?.slice(0, 8)); }}>Delete</Btn>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormInput label="Vehicle ID" value={form.vehicleId} onChange={e => setForm(f => ({ ...f, vehicleId: e.target.value }))} placeholder="Vehicle ID" required />
          <FormSelect label="Maintenance Type" value={form.maintenanceType} onChange={e => setForm(f => ({ ...f, maintenanceType: e.target.value }))}>
            {MAINTENANCE_TYPES.map(t => <option key={t} value={t}>{t.replace(/_/g, ' ')}</option>)}
          </FormSelect>
          <DateInput label="Maintenance Date" value={form.maintenanceDate} onChange={e => setForm(f => ({ ...f, maintenanceDate: e.target.value }))} required />
          <FormInput label="Odometer Reading (km)" type="number" value={form.odometerReading} onChange={e => setForm(f => ({ ...f, odometerReading: e.target.value }))} placeholder="km" min="0" />
          <FormTextarea className="sm:col-span-2" label="Description" value={form.description} onChange={e => setForm(f => ({ ...f, description: e.target.value }))} placeholder="What was done" rows={2} />
          <FormInput label="Cost Amount" type="number" value={form.costAmount} onChange={e => setForm(f => ({ ...f, costAmount: e.target.value }))} placeholder="0.00" min="0" />
          <FormInput label="Currency" value={form.currency} onChange={e => setForm(f => ({ ...f, currency: e.target.value }))} placeholder="TZS" />
          <DateInput label="Next Service Date" value={form.nextServiceDate} onChange={e => setForm(f => ({ ...f, nextServiceDate: e.target.value }))} />
          <FormInput label="Next Service Odometer (km)" type="number" value={form.nextServiceOdometer} onChange={e => setForm(f => ({ ...f, nextServiceOdometer: e.target.value }))} placeholder="km" min="0" />
          {divisions.length > 1 && (
