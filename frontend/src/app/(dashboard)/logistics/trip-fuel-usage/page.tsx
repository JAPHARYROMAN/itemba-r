...
import { Card, PageHeader, Modal, Btn, PageToolbar, FormInput, FormSelect, FormTextarea, DateInput, PageSpinner, ConfirmDialog } from '@/components/ui';
...
const EMPTY_FORM = { tripId: '', vehicleId: '', fuelSource: 'PETROL_STATION', litres: '', unitPrice: '', totalCost: '', odometerBefore: '', odometerAfter: '', fuelDate: '', notes: '' };
...
export default function TripFuelUsagePage() {
  ...
  useEffect(() => {
    if (!companyId) { setDivisions([]); setDivisionId(''); setTrips([]); setVehicles([]); return; }
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/backend/logistics/trip-fuel-usage?companyId=${companyId}&page=1&limit=20`);
  ...
  async function handleSave() {
    ...
    try {
      ...
      const url = isEdit ? `/api/backend/logistics/trip-fuel-usage/${(modal as any).id}` : '/api/backend/logistics/trip-fuel-usage';
  ...
  async function handleDelete() {
    if (!deleteId) return;
    try {
      const res = await fetch(`/api/backend/logistics/trip-fuel-usage/${deleteId}`, {
        method: 'DELETE',
    ...
    <div className="p-6 space-y-4">
          ...
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>Record #</th>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>Trip #</th>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>Vehicle Plate</th>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>Fuel Source</th>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>Litres</th>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>Unit Price</th>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>Total Cost</th>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>Odo Before</th>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>After</th>
              ...
              <tbody>
                {data.data.length === 0 ? (
                  <tr><td colSpan={11} className="px-4 py-8 text-center text-sm" style={{ color: 'var(--aurora-text-muted)' }}>No fuel records found.</td></tr>
                ) : data.data.map((f: any) => (
                  <tr key={f.id} className="border-b border-slate-50 hover:bg-slate-50">
                    <td className={`${tdCls} font-medium`} style={{ color: 'var(--aurora-text)' }}>{f.fuelRecordNumber ?? f.id?.slice(0, 8)}</td>
                    <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>{f.trip?.tripNumber ?? f.tripId ?? '—'}</td>
                    <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>{f.vehicle?.registrationNumber ?? f.vehicleId ?? '—'}</td>
                    ...
                    <td className={tdCls}>
                      <div className="flex gap-3">
                        ...
                        <Btn variant="danger" size="xs" onClick={() => { setDeleteId(f.id); setDeleteLabel(f.fuelRecordNumber || f.id?.slice(0, 8)); }}>Delete</Btn>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormSelect label="Trip" value={form.tripId} onChange={e => updateForm({ tripId: e.target.value })} required placeholder="— Select Trip —">
            {trips.map(t => <option key={t.id} value={t.id}>{t.tripNumber} – {t.origin} → {t.destination}</option>)}
          </FormSelect>
          <FormSelect label="Vehicle" value={form.vehicleId} onChange={e => updateForm({ vehicleId: e.target.value })} placeholder="— Select Vehicle —">
            {vehicles.map(v => <option key={v.id} value={v.id}>{v.vehicleCode} – {v.registrationNumber}</option>)}
          </FormSelect>
          <FormSelect label="Fuel Source" value={form.fuelSource} onChange={e => updateForm({ fuelSource: e.target.value })} required>
            {FUEL_SOURCES.map(s => <option key={s} value={s}>{s.replace(/_/g, ' ')}</option>)}
          </FormSelect>
          <DateInput label="Fuel Date" value={form.fuelDate} onChange={e => updateForm({ fuelDate: e.target.value })} required />
          <FormInput label="Litres" type="number" value={form.litres} onChange={e => updateForm({ litres: e.target.value })} placeholder="0.00" min="0" step="0.01" required />
          <FormInput label="Unit Price (TZS)" type="number" value={form.unitPrice} onChange={e => updateForm({ unitPrice: e.target.value })} placeholder="0.00" min="0" />
          <FormInput className="sm:col-span-2" label="Total Cost (auto-computed)" type="number" value={form.totalCost} onChange={e => setForm(f => ({ ...f, totalCost: e.target.value }))} placeholder="0.00" min="0" />
          <FormInput label="Odometer Before" type="number" value={form.odometerBefore} onChange={e => updateForm({ odometerBefore: e.target.value })} placeholder="km" min="0" />
          <FormInput label="Odometer After" type="number" value={form.odometerAfter} onChange={e => updateForm({ odometerAfter: e.target.value })} placeholder="km" min="0" />
          {divisions.length > 1 && (
            <FormSelect label="Division" value={divisionId} onChange={e => setDivisionId(e.target.value)}>
              {divisions.map(d => <option key={d.id} value={d.id}>{d.name}</option>)}
            </FormSelect>
          )}
          <FormTextarea className="sm:col-span-2" label="Notes" value={form.notes} onChange={e => updateForm({ notes: e.target.value })} placeholder="Optional notes…" rows={3} />
        </div>
      </Modal>
