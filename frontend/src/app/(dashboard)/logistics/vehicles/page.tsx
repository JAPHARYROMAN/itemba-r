...
const VEHICLE_TYPES = ['TRUCK','TRAILER','PICKUP','VAN','MOTORCYCLE','TRACTOR','HEAVY_EQUIPMENT','OTHER'];
...
const EMPTY_FORM = { vehicleCode: '', registrationNumber: '', vehicleType: 'TRUCK', make: '', model: '', year: '', fuelType: 'DIESEL', currentOdometer: '', insuranceExpiryDate: '', roadLicenseExpiryDate: '', inspectionExpiryDate: '', status: 'ACTIVE', notes: '' };
...
export default function VehiclesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/backend/logistics/vehicles?companyId=${companyId}&page=1&limit=20`);
  ...
  function openEdit(row: any) {
    setForm({
      ...
      roadLicenseExpiryDate: row.roadLicenseExpiryDate ? row.roadLicenseExpiryDate.slice(0, 10) : '',
  ...
  async function handleSave() {
    ...
    try {
      ...
      const url = isEdit ? `/api/backend/logistics/vehicles/${(modal as any).id}` : '/api/backend/logistics/vehicles';
  ...
  async function handleDelete() {
    if (!deleteId) return;
    try {
      const res = await fetch(`/api/backend/logistics/vehicles/${deleteId}`, {
        method: 'DELETE',
    ...
    <div className="p-6 space-y-4">
      ...
      {companyId && !loading && (
        <Card className="overflow-hidden">
          <div className="px-4 py-3 border-b border-slate-100 text-xs" style={{ color: 'var(--aurora-text-muted)' }}>{data.total} vehicles</div>
          <div className="overflow-x-auto">
            <table className="w-full">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>Code</th>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>Plate Number</th>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>Type</th>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>Make / Model</th>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>Fuel Type</th>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>Odometer</th>
                  <th className={thCls} style={{ color: 'var(--aurora-text-muted)' }}>Insurance Expiry</th>
              ...
              <tbody>
                {data.data.length === 0 ? (
                  <tr><td colSpan={9} className="px-4 py-8 text-center text-sm" style={{ color: 'var(--aurora-text-muted)' }}>No vehicles found.</td></tr>
                ) : data.data.map((v: any) => (
                  <tr key={v.id} className="border-b border-slate-50 hover:bg-slate-50">
                    <td className={`${tdCls} font-medium`} style={{ color: 'var(--aurora-text)' }}>{v.vehicleCode}</td>
                    <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>{v.registrationNumber}</td>
                    ...
                    <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>{v.fuelType ?? '—'}</td>
                    <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>{v.currentOdometer ?? '—'}</td>
                    <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>{v.insuranceExpiryDate ? fmtDate(v.insuranceExpiryDate) : '—'}</td>
                    <td className={tdCls}><StatusBadge status={v.status} /></td>
                    <td className={tdCls}>
                      <div className="flex gap-3">
                        <Btn variant="ghost" size="xs" onClick={() => openEdit(v)}>Edit</Btn>
                        <Btn variant="danger" size="xs" onClick={() => { setDeleteId(v.id); setDeleteLabel(v.vehicleCode || v.registrationNumber); }}>Delete</Btn>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormInput label="Vehicle Code" value={form.vehicleCode} onChange={e => setForm(f => ({ ...f, vehicleCode: e.target.value }))} placeholder="e.g. VEH-001" required />
          <FormInput label="Plate Number" value={form.registrationNumber} onChange={e => setForm(f => ({ ...f, registrationNumber: e.target.value }))} placeholder="e.g. T 123 ABC" required />
          <FormSelect label="Vehicle Type" value={form.vehicleType} onChange={e => setForm(f => ({ ...f, vehicleType: e.target.value }))} required>
            {VEHICLE_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
          </FormSelect>
          <FormSelect label="Fuel Type" value={form.fuelType} onChange={e => setForm(f => ({ ...f, fuelType: e.target.value }))}>
            {FUEL_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
          </FormSelect>
          <FormInput label="Make" value={form.make} onChange={e => setForm(f => ({ ...f, make: e.target.value }))} placeholder="e.g. Isuzu" />
          <FormInput label="Model" value={form.model} onChange={e => setForm(f => ({ ...f, model: e.target.value }))} placeholder="e.g. FVZ" />
          <FormInput label="Year" type="number" value={form.year} onChange={e => setForm(f => ({ ...f, year: e.target.value }))} placeholder="e.g. 2020" min="1900" max="2100" />
          <FormInput label="Odometer (km)" type="number" value={form.currentOdometer} onChange={e => setForm(f => ({ ...f, currentOdometer: e.target.value }))} placeholder="km" min="0" />
          <DateInput label="Insurance Expiry" value={form.insuranceExpiryDate} onChange={e => setForm(f => ({ ...f, insuranceExpiryDate: e.target.value }))} />
          <DateInput label="Road License Expiry" value={form.roadLicenseExpiryDate} onChange={e => setForm(f => ({ ...f, roadLicenseExpiryDate: e.target.value }))} />
          <DateInput label="Inspection Expiry" value={form.inspectionExpiryDate} onChange={e => setForm(f => ({ ...f, inspectionExpiryDate: e.target.value }))} />
