...
export default function RoutesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/backend/logistics/routes?companyId=${companyId}&page=1&limit=20`,
      );
  ...
  async function handleSave() {
    ...
    try {
      ...
      const url = isEdit
        ? `/api/backend/logistics/routes/${(modal as any).id}`
        : '/api/backend/logistics/routes';
  ...
  async function handleDelete() {
    if (!deleteId) return;
    try {
      await backendDelete(`/logistics/routes/${deleteId}`);
                ...
                {data.data.length === 0 ? (
                  ...
                  data.data.map((r: any) => (
                    <tr key={r.id} className="border-b border-slate-50 hover:bg-slate-50">
                      ...
                      <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>
                        {r.standardRate != null ? fmtCurrency(r.standardRate) : '—'}
                      </td>
                      <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>
                        {r.currency ?? 'TZS'}
                      </td>
                      ...
                      <td className={tdCls}>
                        <div className="flex gap-3">
                          ...
                          <Btn
                            ...
                            onClick={() => {
                              setDeleteId(r.id);
                              setDeleteLabel(r.name || r.routeCode);
                            }}
                          >
                            Delete
                          </Btn>
