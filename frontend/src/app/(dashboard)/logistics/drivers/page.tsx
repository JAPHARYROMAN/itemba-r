...
export default function DriversPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/backend/logistics/drivers?companyId=${companyId}&page=1&limit=20`,
      );
  ...
  async function handleSave() {
    ...
    try {
      ...
      const url = isEdit
        ? `/api/backend/logistics/drivers/${(modal as any).id}`
        : '/api/backend/logistics/drivers';
  ...
  async function handleDelete() {
    if (!deleteId) return;
    try {
      await backendDelete(`/logistics/drivers/${deleteId}`);
                ...
                {data.data.length === 0 ? (
                  ...
                  data.data.map((d: any) => (
                    <tr key={d.id} className="border-b border-slate-50 hover:bg-slate-50">
                      ...
                      <td
                        className={`${tdCls} ${d.licenseExpiryDate && new Date(d.licenseExpiryDate) < new Date() ? 'text-red-600 font-semibold' : ''}`}
                        style={
                      ...
                      <td className={tdCls}>
                        <div className="flex gap-3">
                          <Btn variant="ghost" size="xs" onClick={() => openEdit(d)}>
                            Edit
                          </Btn>
                          <Btn
                            variant="danger"
                            ...
                            onClick={() => {
                              setDeleteId(d.id);
                              setDeleteLabel(d.fullName || d.driverCode);
                            }}
                          >
                            Delete
                          </Btn>
      ...
      >
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <FormInput
            ...
            onChange={(e) => setForm((f) => ({ ...f, driverCode: e.target.value }))}
