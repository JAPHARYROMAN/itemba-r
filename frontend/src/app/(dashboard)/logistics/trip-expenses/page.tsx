...
export default function TripExpensesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/backend/logistics/trip-expenses?companyId=${companyId}&page=1&limit=20`,
      );
  ...
  async function handleSave() {
    ...
    try {
      ...
      const url = isEdit
        ? `/api/backend/logistics/trip-expenses/${(modal as any).id}`
        : '/api/backend/logistics/trip-expenses';
  ...
  async function handleDelete() {
    if (!deleteId) return;
    try {
      await backendDelete(`/logistics/trip-expenses/${deleteId}`);
                  ...
                  data.data.map((e: any) => (
                    <tr key={e.id} className="border-b border-slate-50 hover:bg-slate-50">
                      ...
                      <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>
                        {e.trip?.tripNumber ?? e.tripId ?? '—'}
                      </td>
                      <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>
                        {e.expenseType?.replace(/_/g, ' ') ?? '—'}
                      </td>
                      <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>
                        {e.amount != null ? fmtCurrency(e.amount, e.currency) : '—'}
                      </td>
                      <td className={tdCls} style={{ color: 'var(--aurora-text)' }}>
                        {e.currency ?? 'TZS'}
                      </td>
                      ...
                      <td className={tdCls}>
                        <div className="flex gap-3">
                          <Btn variant="ghost" size="xs" onClick={() => openEdit(e)}>
                            Edit
                          </Btn>
                          <Btn
                            variant="danger"
                            ...
                            onClick={() => {
                              setDeleteId(e.id);
                              setDeleteLabel(e.expenseNumber || e.id?.slice(0, 8));
                            }}
                          >
                            Delete
                          </Btn>
