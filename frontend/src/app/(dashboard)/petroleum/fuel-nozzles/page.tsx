...
interface FuelPump {
  ...
  pumpName: string;
  pumpCode: string;
  tank?: { id: string; tankCode: string; tankName: string; productId: string } | null;
}
...
function fmtNum(n: number | string | null | undefined) {
  return new Intl.NumberFormat('en-US', { maximumFractionDigits: 3 }).format(Number(n) || 0);
}
...
function NozzleModal({
  ...
  nozzle: FuelNozzle | null;
  companies: Company[];
  onClose: () => void;
  onSaved: (nozzle: FuelNozzle) => void;
}) {
  const [companyId, setCompanyId] = useState(nozzle?.companyId ?? '');
  const [branchId, setBranchId] = useState(nozzle?.branchId ?? '');
  const [pumpId, setPumpId] = useState(nozzle?.pumpId ?? '');
  ...
  useEffect(() => {
    ...
    fetch(`/api/backend/petroleum/fuel-pumps/branch/${branchId}`)
      .then((r) => r.json())
      .then((j) => setPumps(unwrapList<FuelPump>(j)))
      .catch(() => setPumps([]));
  }, [branchId]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const selectedPump = pumps.find((pump) => pump.id === pumpId);
    const tankId = selectedPump?.tankId ?? selectedPump?.tank?.id;
    const productId = selectedPump?.tank?.productId;

    if (!companyId || !branchId || !pumpId || !nozzleCode || !nozzleName || openingMeter === '') {
      setError('All required fields must be filled');
      return;
    }

    if (!selectedPump || !tankId || !productId) {
      setError('Selected pump must have an assigned tank before adding a nozzle');
      return;
    }
    ...
    setError('');
    try {
      const meter = Number(openingMeter) || 0;
      const body = {
        nozzleCode,
        nozzleName,
        companyId: selectedPump.companyId ?? companyId,
        branchId: selectedPump.branchId ?? branchId,
        pumpId,
        tankId,
        productId,
        openingMeter: meter,
        currentMeterReading: meter,
      };
      const url = nozzle
        ? `/api/backend/petroleum/fuel-nozzles/${nozzle.id}`
        : '/api/backend/petroleum/fuel-nozzles';
      ...
      onSaved(unwrapRecord<FuelNozzle>(await res.json()));
    } catch (err: unknown) {
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h2 className="text-base font-semibold text-slate-900">
            {nozzle ? 'Edit Nozzle' : 'New Fuel Nozzle'}
          </h2>
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <input
                ...
                value={openingMeter}
                onChange={(e) =>
                  setOpeningMeter(e.target.value === '' ? '' : Number(e.target.value))
                }
                className={fieldCls}
                placeholder="0.000"
              />
...
export default function FuelNozzlesPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      const res = await fetch(`/api/backend/petroleum/fuel-nozzles/pump/${targetPumpId}`);
      if (!res.ok) throw new Error('Failed to load nozzles');
      setNozzles(unwrapList<FuelNozzle>(await res.json()));
    } catch (err: unknown) {
    ...
    <div className="p-6 space-y-5">
                ...
                <tbody className="divide-y divide-slate-100">
                  {nozzles.map((n) => (
                    <tr key={n.id} className="hover:bg-slate-50">
                      ...
                      <td className={tdCls}>
                        {n.tank
                          ? `${n.tank.tankCode ?? ''}${n.tank.tankCode ? ' - ' : ''}${
                              n.tank.tankName
                            }`
                          : '-'}
                      </td>
      ...
      {!pumpId && !loading && (
        <div className="text-center py-10 text-sm text-slate-400">
          Select a company, branch, and pump to view nozzles.
        </div>
      )}
      ...
      {modalOpen && (
        <NozzleModal
          ...
          onSaved={(savedNozzle) => {
            setModalOpen(false);
            setEditing(null);
            setCompanyId(savedNozzle.companyId);
            setBranchId(savedNozzle.branchId);
            setPumpId(savedNozzle.pumpId);
            setNozzles((current) => {
              const next = current.filter((nozzle) => nozzle.id !== savedNozzle.id);
              return [savedNozzle, ...next];
            });
          }}
        />
      )}
