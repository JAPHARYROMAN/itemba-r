...
function PumpModal({ pump, companies, onClose, onSaved }: {
  pump: FuelPump | null; companies: Company[];
  ...
  useEffect(() => {
    if (branchId) {
      fetch(`/api/backend/petroleum/fuel-tanks/branch/${branchId}`).then(r => r.json()).then(j => setTanks(j.data ?? j));
    }
  }, [branchId]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!companyId || !branchId || !tankId || !pumpCode || !pumpName) { setError('All required fields must be filled'); return; }
    setSaving(true); setError('');
    try {
      const body = { pumpCode, pumpName, tankId, branchId, companyId };
      const url = pump ? `/api/backend/petroleum/fuel-pumps/${pump.id}` : '/api/backend/petroleum/fuel-pumps';
      const res = await fetch(url, { method: pump ? 'PATCH' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    ...
    <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-100">
          <h2 className="text-base font-semibold text-slate-900">{pump ? 'Edit Fuel Pump' : 'New Fuel Pump'}</h2>
        ...
        <form onSubmit={handleSubmit} className="px-6 py-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input required value={pumpCode} onChange={e => setPumpCode(e.target.value)} className={fieldCls} placeholder="PMP-001" />
            </div>
            <div>
              <label className={labelCls}>Pump Name *</label>
              <input required value={pumpName} onChange={e => setPumpName(e.target.value)} className={fieldCls} placeholder="Pump 1" />
            </div>
...
export default function FuelPumpsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetch(`/api/backend/petroleum/fuel-pumps/branch/${branchId}`);
    ...
    <div className="p-6 space-y-5">
      ...
      {!loading && branchId && (
        <Card className="overflow-hidden">
          {pumps.length === 0 ? (
            <p className="text-sm text-slate-400 text-center py-10">No pumps found for this branch.</p>
          ) : (
                ...
                <tbody className="divide-y divide-slate-100">
                  {pumps.map(p => (
                    <tr key={p.id} className="hover:bg-slate-50">
                      ...
                      <td className={tdCls}>{p.tank?.tankName ?? '—'}</td>
                      <td className={tdCls}>{p.branch?.name ?? '—'}</td>
                      <td className={tdCls}><Badge status={p.status} /></td>
                      <td className="px-4 py-2 text-right">
                        <button onClick={() => { setEditing(p); setModalOpen(true); }} className="text-xs text-indigo-600 hover:text-indigo-800 mr-3">Edit</button>
      ...
      {!branchId && !loading && <div className="text-center py-10 text-sm text-slate-400">Select a company and branch to view pumps.</div>}
