...
export function Modal({
  ...
  return (
    ...
    >
      ...
      />
      <div
        className={`relative w-full ${SIZE_CLASS[size]} rounded-aurora-lg overflow-hidden ${isClosing ? 'animate-scale-out' : 'animate-scale-in'} ${className}`}
        style={{
        ...
        {(title || description) && (
          ...
          >
            <div>
              ...
              {description && (
                <p className="text-sm mt-0.5" style={{ color: 'var(--aurora-text-muted)' }}>
                  {description}
                </p>
              )}
            </div>
            <button
              onClick={onClose}
              className="w-8 h-8 flex items-center justify-center rounded-lg transition-colors ml-3 flex-shrink-0"
