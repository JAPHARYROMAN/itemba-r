...
vi.mock('@/hooks/use-auth', () => ({
  useAuth: () => ({ user: { ...auth, companyId: 'company' } }),
}));
...
function Example() {
  ...
  return (
    <>
      <button onClick={() => setApp(app === 'invoice' ? 'cash' : 'invoice')}>Switch</button>
      {app === 'invoice' ? <InvoiceView /> : <p>Cash Desk</p>}
    </>
  );
}
beforeEach(() => {
  auth.id = 'alice';
  auth.permissions = ['invoice_desk.view'];
});
