...
export class CompanyScopeService {
  ...
  ): Promise<Prisma.BankAccountWhereInput> {
    if (this.isGroupScoped(user)) {
  ...
  private async getAccessibleCompanyAccess(user: AuthUser): Promise<Array<{ companyId: string; accessLevel: AccessLevel }>> {
    ...
    for (const entry of user.companyAccess ?? []) {
      access.set(entry.companyId, this.maxAccess(access.get(entry.companyId), entry.accessLevel as AccessLevel));
    }

    if (user.companyAccess !== undefined) {
      return Array.from(access.entries()).map(([companyId, accessLevel]) => ({ companyId, accessLevel }));
    }
