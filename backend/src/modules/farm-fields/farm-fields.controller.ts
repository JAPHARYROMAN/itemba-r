...
export class FarmFieldsController {
  ...
  findAll(@Query('farmId') farmId?: string, @Query('companyId') companyId?: string, @Query('page') page?: string, @Query('limit') limit?: string, @CurrentUser() user: AuthUser = undefined as unknown as AuthUser) {
    return this.service.findAll(farmId, companyId, page ? +page : 1, limit ? +limit : 20, user);
  }
