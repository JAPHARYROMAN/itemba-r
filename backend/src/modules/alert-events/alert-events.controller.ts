...
export class AlertEventsController {
  ...
  acknowledge(@Param('id') id: string, @Body() body: { comment?: string }, @CurrentUser() user: AuthUser) {
    return this.service.acknowledge(id, body, user);
  }

  @Patch(':id/resolve')
  @RequirePermissions('alert_events.resolve')
  resolve(@Param('id') id: string, @Body() body: { comment?: string }, @CurrentUser() user: AuthUser) {
    return this.service.resolve(id, body, user);
  }

  @Patch(':id/dismiss')
  @RequirePermissions('alert_events.dismiss')
  dismiss(@Param('id') id: string, @Body() body: { comment?: string }, @CurrentUser() user: AuthUser) {
