...
import {
  MediateDisputeDto, ReferCmaDisputeDto, ResolveDisputeDto, UpdateEmploymentDisputeDto,
...
export class EmploymentDisputesController {
  ...
  findAll(@Query() q: Record<string, string>) {
    return this.service.findAll({ page: q.page ? Number(q.page) : undefined, limit: q.limit ? Number(q.limit) : undefined, companyId: q.companyId, employeeId: q.employeeId, status: q.status });
  }
  ...
  update(@Param('id') id: string, @Body() dto: UpdateEmploymentDisputeDto, @CurrentUser() user: AuthUser) {
    return this.service.update(id, dto, user.id);
  }

  @Patch(':id/mediate') @RequirePermissions('employees.update')
  mediate(@Param('id') id: string, @Body() dto: MediateDisputeDto, @CurrentUser() user: AuthUser) {
    return this.service.startMediation(id, dto, user.id);
  }

  @Patch(':id/refer-cma') @RequirePermissions('employees.update')
  referCma(@Param('id') id: string, @Body() dto: ReferCmaDisputeDto, @CurrentUser() user: AuthUser) {
    return this.service.referToCma(id, dto, user.id);
  }

  @Patch(':id/resolve') @RequirePermissions('employees.update')
  ...
  @Patch(':id/withdraw') @RequirePermissions('employees.update')
