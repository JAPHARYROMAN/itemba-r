...
// The permission vocabulary and the system-role matrices. Extracted so they can
// be asserted by tests without importing this file, which runs main() on import.
import { ALL_PERMISSIONS, ROLES } from './permission-matrix';
import { seedMsaidiziUserRole } from './msaidizi-user-role';
...
async function main() {
  ...
  // Provisioned outside the matrix loop above, deliberately: MSAIDIZI_USER is a
  // non-system role, so the full replace that just ran must not reach it. It is
  // created once and then left to whoever administers it — see ./msaidizi-user-role.
  await seedMsaidiziUserRole(prisma);
